...
DEPS="$PERSIST/pydeps-gamemaster"
...
if [ "${1:-}" = "train" ] && [ ! -f "$DEPS/.train-installed" ]; then
  python3 -m pip install --target "$DEPS" -r "$REPO/requirements-train.txt"
  touch "$DEPS/.train-installed"
fi
